...
\item We characterize the \textbf{minimal architecture for grokking}: a depth-2
  matrix factorization with ReLU groks 18$\times$ faster than the full
  transformer, while the linear variant never groks despite spectral
  compression. Nonlinearity is necessary; attention is not.
\item We show that \textbf{Fourier structure emerges after generalization},
  not before it: fine-grained trajectory analysis (1{,}002 checkpoints)
  reveals a $\sim$1{,}000-epoch lag between test accuracy crossing 90\% and
  Fourier alignment crossing 0.30. The gradient's AGOP discovers Fourier
  structure $\sim$10{,}000 epochs before the weights do, with an unexpected
  double-peak pattern during the phase transition.
\item We demonstrate that \textbf{weight-space SVD predicts causal geometry}
  without optimization: SVD of the feedforward product matrix achieves
  IIA~=~0.999 where DAS achieves 0.150, and that the \textbf{causal subspace
  identity is gauge freedom}---10 seeds produce orthogonal subspaces
  (overlap $\approx 0.008$) that are all equally valid.
\item We test \textbf{stratum classification} (spectral profiling of weight
  matrices) and show it works for single-task grokking models but fails
  completely on pretrained GPT-2 due to superposition. Functional diagnostics
  (equivariance) outperform structural diagnostics (spectral profile).
\end{enumerate}
...
Second, \textbf{attention is not necessary}: DMF + ReLU groks 18$\times$
faster than the full transformer. The softmax attention mechanism adds
enormous overhead to the grokking process without being required for the
underlying algorithm. This suggests that for modular arithmetic, attention's
role is to implement a computation that a feedforward network with
nonlinearity can achieve more directly.
...
training---while DAS achieves only 0.150 after 1{,}000 gradient steps.
The weight matrices directly encode the causal structure.
...
on the wrong operation's labels---destroy it? We test three grokked models
by retraining them with shuffled labels from a different operation.
...
We test whether the singular value structure of weight matrices provides
an alternative lens on causal geometry. Following ideas from random matrix
theory, we classify each head's OV and QK circuits into four \emph{strata}
based on their spectral profile:
...
\midrule
$\mathcal{M}_1$ & gap ratio $> 3$, $k_{90} \leq 2$ & Single dominant direction \\
$\mathcal{M}_k$ & top-8 cumvar $> 0.85$, $k_{90} \leq 8$ & Low-rank subspace \\
$\mathcal{M}_\Sigma$ & erank $< 0.3 \cdot \text{dim}$ & Moderate rank \\
$\mathcal{M}_D$ & none of the above & Full-rank / distributed \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Grokked toy models: strata reflect algorithm structure.}
In grokked 1-layer models ($p = 113$, $d_\text{model} = 128$), all four
attention heads have OV $\in \mathcal{M}_k$ (erank 2.1--6.2) and
QK $\in \mathcal{M}_1$ (single dominant singular value with gap ratio $> 3$).
The FF circuit is $\mathcal{M}_k$ (erank 5.5). This is consistent with the
Fourier mechanism: QK selects a single frequency pair ($\mathcal{M}_1$), while
OV operates in the broader subspace of active Fourier modes
($\mathcal{M}_k$).

\paragraph{Pretrained GPT-2: strata have no discriminative power.}
In GPT-2-small ($d_\text{model} = 768$, $d_\text{head} = 64$), 143 of 144
attention heads have OV $\in \mathcal{M}_\Sigma$ regardless of their
function. The single $\mathcal{M}_1$ head (L11H8, erank 1.18) is
\emph{not} a known circuit head. Known circuit heads---IOI name movers
(erank 60.1--60.4), induction heads (erank 51.5--61.0), s-inhibition heads
(erank 57.0--59.9)---are indistinguishable from non-circuit heads by
stratum. Circuit heads have \emph{higher} average OV erank (54.6)
than non-circuit heads (49.5), the opposite of what the theory predicts.
...
\begin{tabular}{lcc}
\toprule
& Circuit heads ($n{=}40$) & Non-circuit ($n{=}104$) \\
\midrule
OV erank (mean) & 54.6 & 49.5 \\
OV gap ratio (mean) & 1.24 & 1.39 \\
QK erank (mean) & 53.0 & 46.9 \\
\bottomrule
\end{tabular}
\caption{Circuit vs.\ non-circuit heads in GPT-2. Circuit heads have
\emph{higher} effective rank---more distributed, not less. The stratum
classification fails to distinguish them.}
\label{tab:gpt2_strata}
\end{table}

\paragraph{Why strata fail at scale.}
The failure is expected under superposition. GPT-2 solves hundreds of tasks
simultaneously; a single head participating in IOI, induction, and
greater-than needs different principal components for each task. Its OV
matrix is the superposition of many low-rank contributions, producing high
effective rank ($\mathcal{M}_\Sigma$) even though each individual
task-circuit may be low-rank. Grokking models do not superpose---they
solve exactly one problem in 128 dimensions using 2--8 dimensions of that
space. Strata are a valid description of \emph{single-task} models but do
not generalize to pretrained models under superposition.

The most promising rescue is \textbf{task-conditioned strata}: project each
head's OV matrix into the task-specific DAS subspace, then classify the
projected matrix's stratum. If IOI name movers are $\mathcal{M}_1$ when
projected into the IOI subspace, the theory is rehabilitated at the cost
of requiring task-specific measurement. We leave this to future work.

\paragraph{Strata vs.\ equivariance.}
The equivariance diagnostic (\S\ref{sec:results}) cleanly separates the
three-class partition without any spectral analysis. The stratum
classification adds mechanistic detail to what equivariance already captures,
but does not provide discriminative power beyond it. For the atlas, the lesson
is methodological: \textbf{functional diagnostics (equivariance) outperform
structural diagnostics (spectral profile)} for classifying causal geometry.
...
diagnostics apply regardless. The structured VAE introduces a model-selection
...
connection between the structured VAE latent space and the Fourier
...
remain untested. The un-training experiments (\S\ref{sec:un_training}) use
only three operations---broader coverage would clarify whether composite
moduli systematically produce more robust grokked structure.
