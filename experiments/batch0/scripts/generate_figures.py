...
def fig14_k_sweep() -> None:
    ...
    colors_ksweep = {
        ...
        "Comp. addition": "#1b7837",
        "Polynomial": "#e08214",
        "Affine": "#c51b7d",
        "Abs. difference": "#7b3294",
    }
    styles = {
        "Multiplication": "o-",
        "Comp. addition": "s-",
    ...
    markers = {"Multiplication": "o", "Comp. addition": "s",
               "Polynomial": "D", "Affine": "v", "Abs. difference": "^"}
    for name, data in K_SWEEP_DATA.items():
        ks = [d[0] for d in data]
        iias = [d[1] for d in data]
        ax.plot(ks, iias, "-", color=colors_ksweep[name], linewidth=1, alpha=0.5, zorder=2)
        ax.scatter(ks, iias, c=colors_ksweep[name], s=40, marker=markers[name],
...
def fig20_real_circles() -> None:
    ...
    sorted_circles = sorted(circles.items(), key=lambda x: x[1]["equiv"], reverse=True)
    ...
    for idx, (label, data) in enumerate(sorted_circles):
        ...
        if data["all_2d"] is not None:
            pts = data["all_2d"]
            ax.scatter(pts[:, 0], pts[:, 1], c="#cccccc", s=1, alpha=0.15, zorder=1)

        ax.scatter(centroids[:, 0], centroids[:, 1], c=color, s=12, alpha=0.8,
        ...
        ax.set_title(f"{label}\neq={eq:.3f}{cv_str}", fontsize=7, pad=3)
        for spine in ax.spines.values():
            spine.set_color("#dddddd")
    ...
    fig.suptitle("Real DAS 2D projections — label centroids", fontsize=10, y=1.01)
...
def fig23b_k_sweep_equiv_colored() -> None:
    ...
    interesting = ["multiplication", "composite_addition", "polynomial",
                   "affine", "abs_diff", "cubing", "power", "squaring",
                   "bitwise_xor", "division", "subtraction", "max_ab",
                   "sum_of_squares", "cubic_sum"]
    filtered = {k: v for k, v in sweeps.items() if v["op"] in interesting}
    if not filtered:
        filtered = dict(list(sweeps.items())[:10])
    ...
    for draw_idx, (label, data) in enumerate(sorted_items):
        ...
        iias_sorted = [p[1] for p in sorted_pairs]
        z = 2 + draw_idx
        ax.plot(ks_sorted, iias_sorted, "-", color=color, linewidth=1.5, alpha=0.7, zorder=z)
        ax.scatter(ks_sorted, iias_sorted, c=[color], s=35, marker=marker,
                   edgecolors="white", linewidths=0.4, zorder=z + 1,
...
def fig24_three_class_trajectories() -> None:
    ...
    panels = [
        ...
        ("Never groks: Squaring", "squaring", "#c51b7d"),
    ]
