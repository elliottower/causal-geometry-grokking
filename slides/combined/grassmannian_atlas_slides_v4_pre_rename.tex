...
\vspace{0.4em}
\textbf{\hyperlink{sec10b}{10b. pi-SAE (ours)}}\\[0.2em]
...
{\small pi-SAE on GPT-2 language tasks (SVA, greater-than, gender bias, capitals, hypernymy). Detects nonlinear structure that DAS misses.}
...
\hypertarget{sec10b}{}
\section{10b. pi-SAE: sparse nonlinear causal discovery}

%% --- SLIDE: From VAE to pi-SAE ---
\begin{frame}{From pi-VAE to pi-SAE: sparsity matters}
...
\end{column}
\begin{column}{0.48\textwidth}
\textbf{pi-SAE} {\small \textcolor{vae}{(ours)}}\\[0.3em]
{\small Same label-conditional prior, plus \textbf{L1 sparsity} on the latent:}
\vspace{-0.2em}
\[ \mathcal{L} = \mathcal{L}_\text{pi-VAE} + \lambda \|z\|_1 \]
\vspace{-0.5em}
{\small The sparsity constraint matches the Fourier structure of grokked representations --- sparse, structured codes.}
...
{\small Sparsity is not optional. The pi-VAE prior alone cannot recover the causal variable on grokked operations. Adding L1 penalty transforms it from IIA $= 0$ to IIA $= 1$.}
\vspace{0.2em}}}
\end{frame}
...
\vspace{0.3em}
{\small A nonlinear method can learn $z = \theta$ directly and decode back to the circle. \textbf{pi-SAE at $k{=}1$}:}

\vspace{0.3em}
{\small\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lccc@{}}
\toprule
& \textbf{DAS} & \textbf{NL-DAS} & \textbf{pi-SAE} \\
\midrule
...
\begin{frame}{pi-SAE results across all operations}
\vspace{-0.3em}
\centering
...
\textbf{Operation} & \textbf{Grok} & \textbf{Acc} & \textbf{DAS} & \textbf{NL-DAS} & \textbf{pi-SAE} & \textbf{DAS} & \textbf{NL-DAS} & \textbf{pi-SAE} \\
\midrule
...
\vspace{0.3em}
{\small pi-SAE achieves IIA $= 1.0$ on all grokked operations and correctly returns $\approx 0$ on all non-grokked operations. NL-DAS returns 0.6--0.8 on non-grokked operations --- \textbf{false positives}.}
\end{frame}
...
\textbf{Operation} & VAE & SAE & pi-VAE & \textbf{pi-SAE} \\
\midrule
...
\fcolorbox{always}{always!8}{\parbox{0.9\linewidth}{\centering\vspace{0.15em}
  {\small \textbf{pi-SAE} is uniformly best.\\Structured prior + sparsity\\= robust across all tasks.}
\vspace{0.15em}}}
\end{column}
\end{columns}
...
\vspace{0.3em}
\centering
{\small Neither component alone suffices: pi-VAE (no sparsity) $= 0$; plain SAE (no prior) is inconsistent.}
\end{frame}

%% --- SLIDE: NL-DAS vacuity exposed ---
\begin{frame}{pi-SAE exposes NL-DAS vacuity}
...
\vspace{0.5em}
\textbf{pi-SAE correctly rejects:}\\[0.3em]
{\small On the same operations, pi-SAE returns IIA $\leq 0.04$. The reconstruction constraint prevents hallucination.}
...
\begin{tikzpicture}[scale=0.8]
  ...
  \node[font=\scriptsize\bfseries,always] at (3.25,4.2) {pi-SAE};
...
{\small \textbf{Recommendation}: replace NL-DAS with pi-SAE for nonlinear causal discovery. Same expressivity, no vacuity.}
\vspace{0.1em}}}
\end{frame}
...
pi-SAE captures both at $k{=}2$.}\\[0.5em]
...
\toprule
\textbf{Task} & $k$ & \textbf{DAS} & \textbf{NL-D} & \textbf{pi-SAE} & \\
\midrule
...
pi-SAE $k{=}2$ beats DAS $k{=}8$ (0.86--0.89).
}
\end{column}
\end{columns}
\end{frame}
...
pi-SAE (ours) & 0.00 & \textcolor{always}{\textbf{0.98}} & \textcolor{always}{\textbf{0.98}} & \textcolor{always}{\textbf{0.98}} & \textcolor{always}{\textbf{0.98}} & \textcolor{always}{\textbf{0.98}} \\
\bottomrule
\end{tabular}
}
...
\begin{tikzpicture}[xscale=0.55,yscale=3.5]
  ...
  \node[font=\tiny,always,right] at (8.1,0.98) {pi-SAE};
\end{tikzpicture}
...
\vspace{0.3em}
\centering
{\small pi-SAE saturates at $k{=}2$ (intrinsic dimension). DAS climbs linearly --- still hasn't converged at $k{=}8$.}
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.55\textwidth}
{\small The minimum $k$ at which pi-SAE achieves IIA $\geq 0.9$ reveals the \textbf{true intrinsic dimension} of the causal variable:}
...
%% ===================================================================
%% 10c. NLP tasks: pi-SAE on GPT-2 language tasks
%% ===================================================================
\hypertarget{sec10c}{}
\section{10c. Beyond grokking: NLP tasks on GPT-2}

%% --- SLIDE: NLP task results overview ---
\begin{frame}{pi-SAE on GPT-2 language tasks}

\begin{columns}[T]
\begin{column}{0.55\textwidth}
{\small Beyond toy grokking models: apply pi-SAE to \textbf{real NLP tasks} in GPT-2-small at layer 10.}
...
\vspace{0.3em}
{\small Key question: does pi-SAE's \textbf{conservatism} --- only reporting high IIA when genuine low-dimensional structure exists --- hold on language tasks?}
\end{column}
...
\begin{tabular}{l|ccc}
\toprule
\textbf{Task} & \textbf{DAS} & \textbf{NL-DAS} & \textbf{pi-SAE} \\
\midrule
...
\begin{frame}{Interpretation: pi-SAE as a causal structure detector}
...
pi-SAE complements: it \textbf{detects} nonlinear structure that DAS misses (capitals, hypernymy), but DAS has higher IIA on linearly-accessible variables (gender bias).\\[0.2em]
\textbf{Together}: structure detection (pi-SAE) + subspace identification (factorized DAS).}
\end{column}
\end{columns}
\end{frame}
